...
# 「静默数据获取」开关：自动获取循环（订单/在售/待办/通知同步）期间置 True，
# 让 MITM 浏览器以真·无头方式启动，全程不在前台显示、不抢占焦点。
# 用 contextvars 而非全局变量：随 async 任务链自动传播，且不会污染同进程内
# 用户手动操作（/mercari-accounts「打开浏览器」）或出品/删除/改价等有头流程。
_silent_data_fetch: contextvars.ContextVar[bool] = contextvars.ContextVar(
    "mitm_silent_data_fetch", default=False
)


@asynccontextmanager
async def silent_data_fetch_scope() -> AsyncIterator[None]:
    """在此作用域内执行的所有 ``mitm_automation_browser`` 强制无头静默运行。

    供「自动获取」后台循环包裹各子任务使用，不影响其它调用方。
    """
    token = _silent_data_fetch.set(True)
    try:
        yield
    finally:
...
async def mitm_automation_browser(
    ...
    *,
    ...
    若在 ``silent_data_fetch_scope()`` 作用域内调用(自动获取循环)，浏览器一律以
    真·无头方式启动,全程不在前台显示( ``minimized`` 此时自动失效)。
    """
    aid = int(account_id)
    ...
    # 数据获取期间（silent_data_fetch_scope）强制无头：彻底不在前台显示。
    silent = _silent_data_fetch.get()
    use_headless = automation_headless_enabled() or silent
